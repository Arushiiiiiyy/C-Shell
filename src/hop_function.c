...
/*void hop_function(char* user_input,char*home_dir,char *prev_path,int* hopper){
    char* pointer_space=strchr(user_input,' ');
    if(user_input[3]=='\0'){
        return ;
    }
    char curr_work[4096];//pls change respectively
    strcpy(prev_path,getcwd(curr_work,sizeof(curr_work)));
    ...
    while(pointer_space!=NULL && *pointer_space!='\0'){  
        ...
        if(strncmp("..",pointer_space,2)==0){
            ...
            strcpy(home_dir,home_dir_change);
            pointer_space++;
        }
        else if(strncmp("-",pointer_space,1)==0){
            if(*hopper==0){
        ...
        //}
            ...
            char path_initi[4097]={0};
            getcwd(path_initi,sizeof(path_initi));
            char *token=strtok(pointer_space, " /");   
            if (token != NULL) {
                int length=strlen(token);
                if (chdir(token) == -1) {
